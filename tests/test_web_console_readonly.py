...
""",
    ...
    def test_health_and_status_expose_read_only_snapshot(self) -> None:
        ...
        self.assertTrue(history_item["tokensAvailable"])
        self.assertEqual({"in": 11, "out": 22, "available": True}, history_item["tokens"])
        self.assertFalse(history_item["quotaAvailable"])
        self.assertEqual({"window": "", "used": None, "available": False}, history_item["quota"])
    ...
    def test_api_history_and_history_view_show_cycle_change_summary_and_failed_tasks(self) -> None:
        ...
        self.assertIn("Run comparison", history_html)
        self.assertIn("Compare against", history_html)
        self.assertIn("Token / quota", history_html)
    ...
    def test_adapter_normalizes_live_run_contract_from_api_snapshot(self) -> None:
        ...
        retention_group = next(group for group in config["groups"] if group["id"] == "retention")
        self.assertIn("retention.max_days", retention_group["paths"])
        self.assertIn("retention.keep_pending_worktree_runs", retention_group["paths"])
        self.assertIn("retention.max_days", config["schema"])
        self.assertEqual("Retention enabled", config["schema"]["retention.enabled"]["label"])
        self.assertEqual(30, config["defaults"]["retention"]["max_days"])
        self.assertTrue(config["values"]["retention"]["keep_pending_worktree_runs"])
        ...
        self.assertTrue({"Project", "Runner", "Quota", "Worktree", "Prompt Paths", "Codex Models", "PM Refresh", "Budget", "Telegram", "Goals", "Retention"}.issubset(group_titles))
        ...
        retention = self.client.get("/api/retention/dry-run").json()
        self.assertTrue(retention["dryRun"])
        self.assertIn("agent_runs", retention["roots"])
        self.assertIn("summary", retention)
        self.assertIn("candidates", retention)
