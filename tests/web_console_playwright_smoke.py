...
""",
    ...
    def test_primary_views_locale_and_mobile_width(self) -> None:
        ...
        try:
            ...
            self._open_view(page, "nav-logs", "logs", "Live tail")
            self.expect(page.get_by_role("button", name="Download filtered logs")).to_be_visible()
            self.expect(page.locator('[data-action="toggle-logs"]')).to_have_text("Pause live tail")
            self._open_view(page, "nav-worktree", "worktree", "Pending merge")
            self.expect(page.get_by_role("button", name="Apply merge")).to_be_visible()
            self.expect(page.get_by_role("button", name="Discard merge")).to_be_visible()
            ...
            self.expect(worktree_overlay).to_contain_text('Type "MERGE WORKTREE" to confirm this worktree action.')
            self.expect(page.get_by_role("button", name="Confirm merge")).to_be_visible()
            page.keyboard.press("Escape")
