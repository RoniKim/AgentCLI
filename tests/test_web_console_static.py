...
class WebConsoleStaticTests(unittest.TestCase):
    ...
    def test_required_views_have_nonblank_render_content(self) -> None:
        render_markers = {
            "renderDashboard": ["Pipeline snapshot", "No log entries yet.", "No goals published yet."],
            "renderPipeline": ["Stage lane", "Current stage output", "Only some stage records were published."],
            "renderLogs": ["Tail filter", "No log entries match the current filter.", "Read-only snapshot"],
            "renderBacklog": ["Work queue", "No task selected.", "No tasks."],
            "renderGoals": ["Goal progress", "Loading the read-only snapshot...", "Read-only API snapshot with browser-local edits for offline annotation only."],
