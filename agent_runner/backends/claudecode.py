            ...
            for attempt in range(max_attempts):
                ...
                if build_enabled:
                    ...
                    if not ok:
                        ...
                        _record_failed_state("build_failed", detail=build_detail[:500], task_status=task_status)
                        save_state(state_path, state)
                ...
                if run_tests:
                    ...
                    if not ok:
                        ...
                        _record_failed_state("test_failed", detail=test_detail[:500], task_status=task_status)
                        save_state(state_path, state)
                ...
                if should_run_fast_web_worktree_regression(repo, fast_regression_files):
                    ...
                    if not fast_ok:
                        ...
                        if (
                            ...
                            continue
                        _record_failed_state("fast_regression_failed", detail=failed_summary[:500], task_status=task_status)
                        save_state(state_path, state)
                        _record_history(
                            next_task.id,
                            next_task.title,
