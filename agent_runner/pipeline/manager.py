...
class PipelineManager:
    ...
    async def run_cycle(self, session: PipelineSession, cycle_idx: int, *, continuous: bool) -> CycleResult:
        if session.has_stop():
            return CycleResult(rc=0, reason="stop_file", done_delta=0)
        ...
        if not self._has_stage("PM"):
            if not session.ensure_backlog():
                return CycleResult(rc=1, reason="pm_disabled_no_backlog", done_delta=0)

        tasks_checked = False

        for stage in self.stages:
            if session.has_stop():
                return CycleResult(rc=0, reason="stop_file", done_delta=session.done_delta)
            ...
            if stage_name != "pm" and not tasks_checked:
                if not session.ensure_tasks_loaded():
                    ...
                    return CycleResult(rc=1, reason="no_tasks", done_delta=0)
                tasks_checked = True

                if not continuous:
                    # behave like legacy: prepare artifacts and stop before executing tasks
                    return CycleResult(rc=0, reason="prepared_only", done_delta=0)
            ...
            if out.status == "skip":
                continue
            if out.status == "stop":
                return CycleResult(rc=out.rc, reason=out.reason or "stop", done_delta=session.done_delta)
            if out.status == "fail":
                return CycleResult(rc=out.rc, reason=out.reason or "failed", done_delta=session.done_delta)
        ...
        return CycleResult(rc=0, reason="ok", done_delta=session.done_delta)
