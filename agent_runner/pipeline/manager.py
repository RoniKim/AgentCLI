...
from .tooling import build_tool_spec, open_tool_server
...
async def _main_async_impl(args: argparse.Namespace) -> int:
    force_utf8_stdio()
    ...
    # Tool backend (MCP stdio) — designed for extensibility.
    # - Current: Codex CLI via "codex mcp-server" or "npx -y @openai/codex@latest mcp-server"
    # - Future: Claude Code (or others) via --tool-command/--tool-args
    tool_spec = build_tool_spec(args)
    ...
    async with open_tool_server(
        MCPServerStdio,
        tool_spec,
        session_timeout_seconds=int(getattr(args, "mcp_timeout_seconds", 120)),
    ) as tool_server:

        mcp_servers = [tool_server] if tool_server is not None else []
        ...
        async def run_qa_if_needed(cycle_idx: int, ran_tasks: bool) -> None:
            ...
            try:
                ...
                # QA failures should be visible to users; do not silently continue.
                try:
                    (run_dir / f"qa_error_cycle_{cycle_idx:03d}.txt").write_text(str(ex) + "\n", encoding="utf-8", errors="replace")
                except Exception:
                    pass
                eprint(f"[QA WARNING] QA step failed: {ex}")
            ...
            for step in range(int(args.iterations)):
                ...
                for attempt in range(max_attempts):
                    ...
                    if build_enabled:
                        ...
                        if not ok:
                            if stop_path.exists():
                                append_cycle_summary(f"{now_iso()} cycle={cycle_idx} stop=stop_file during=build task={next_task.id}")
                                return 0, "stop_file", 0
                    ...
                    if run_tests:
                        ...
                        if not ok:
                            if stop_path.exists():
                                append_cycle_summary(f"{now_iso()} cycle={cycle_idx} stop=stop_file during=test task={next_task.id}")
                                return 0, "stop_file", 0
...
class CycleManager:
    """Class wrapper around the runner orchestration.

    The actual implementation lives in `_main_async_impl`, kept as a single
    function for minimal behavioral churn during refactoring. The class exists
    to provide a clear extension point for future pipeline modularization
    (additional phases/agents, different tooling backends, etc.).
    """
    ...
    async def run(self) -> int:
        return await _main_async_impl(self.args)


async def main_async(args: argparse.Namespace) -> int:
    """Public async entrypoint (compat)."""
    return await CycleManager(args).run()
