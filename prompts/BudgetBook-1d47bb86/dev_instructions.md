...
- Before calling or using ANY existing method/property/component, READ its definition first (method signature, parameter types, return type).
- Never assume API names, parameter order, or return shapes — verify from source.
- Common pitfalls: wrong property names (.Success vs .Ok), tuple vs flat returns, method overloads with different parameter types.
...
Additional guard:
- If you receive a task that is only about PM artifacts or documentation (PROJECT_ANALYSIS.md, REQUIREMENTS/AGENT_TASKS/BACKLOG/NOTES, or .doc/ or .AgentCLI/ only),
